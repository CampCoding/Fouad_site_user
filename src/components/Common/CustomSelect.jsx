  ...
  return (
    <div ref={containerRef} className={`relative w-full ${className}`}>
      <div
        ...
        className={`${selectClassName} flex items-center justify-between h-[38.4px] bg-[#171717] border rounded-[4px] cursor-pointer group transition-all ${
          error
            ? ""
      ...
      >
        <div className="flex-1 px-3 text-start truncate flex items-center gap-2">
          ...
          <span
            className={`${selectedOption ? "text-white" : "text-(--main-color)"} text-sm`}
          >
      ...
      {isOpen && (
        <div className="absolute top-full left-0 z-50 w-full mt-1 bg-[#171717] border border-(--main-color)/30 rounded-[4px] shadow-lg max-h-60 overflow-y-auto">
          {options.length > 0 ? (
            ...
            <div className="px-4 py-2 text-sm text-(--main-color)/50">
              لا يوجد خيارات
            </div>
          )}
        </div>
